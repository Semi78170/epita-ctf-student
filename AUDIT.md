...
Le contrat cible analysé est déployé à l’adresse : 0xed5415679D46415f6f9a82677F8F4E9ed9D1302b.
...
Le fonctionnement général repose sur la validation de conditions spécifiques. Si celles-ci sont respectées, le contrat envoie des fonds à l’utilisateur. L’analyse met en évidence plusieurs points sensibles. Tout d'abord, le résultat dépend d’un calcul interne, potentiellement basé sur des données accessibles. Certaines variables influencent directement le résultat, comme :
- une valeur de prédiction (_guess)
- un identifiant de round (_round)
- un paramètre de validation (_nonce)

Ensuite, le contrat semble également dépendre de sources externes ou semi-prévisibles :
...
Cette version spécifique n’étant pas disponible dans mon environnement local, une modification a été nécessaire. La directive a été adaptée comme cela : pragma solidity ^0.8.19.
Ce changement a permis de rendre le projet compatible avec les versions de compilateur disponibles.
...
Le choix s’est porté sur l’option B (contrat Drainer). Ce choix vient du fait que le sujet impose de faire l’attaque et la distribution en une seule transaction. Utiliser un contrat permet de gérer ça plus facilement et de s’assurer que tout se passe correctement d’un coup.
...
Le contrat implémente également une fonction receive() permettant de recevoir des fonds, nécessaire pour tester la redistribution.

# 6. Tests
Un premier test a consisté à envoyer des fonds au contrat afin de vérifier sa capacité à recevoir des eth. 
...
Cela indique que le contrat cible vérifie une signature ou un payload spécifique avant d’autoriser l’exécution. Cette vérification empêche les appels naïfs et implique que les paramètres doivent être calculés avec précision à partir de données internes ou externes.
...
Les transactions ont été effectuées via des outils en ligne de commande (cast), ce qui a permis de tester directement les interactions avec le contrat déployé.

Les résultats observés ont été cohérents avec les hypothèses formulées, notamment concernant l’existence d’un mécanisme de validation empêchant les appels non conformes.
...
- Une tentative de vérification du contrat sur l’explorateur de blocs qui a échoué en raison d’une non-correspondance du bytecode. Cela est dû à des modifications du code après le déploiement.
...
De plus, l’utilisation d’un oracle externe et de données liées à la blockchain (timestamp, blockhash) complexifie encore davantage la prédiction. Enfin, la concurrence sur le réseau constitue un facteur aggravant, car d’autres acteurs peuvent interagir avec le contrat en parallèle.
